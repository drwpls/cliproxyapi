...
type consumeCodexResetRequest struct {
	...
	// CreditID is optional; when empty the handler redeems the first available
	// (unused) credit returned by the list endpoint.
	CreditID string `json:"credit_id"`
}

// ConsumeCodexResetCredit redeems one rate-limit reset credit for a Codex
// account. When no credit_id is supplied it looks up the first unused credit
// and redeems that one.
//
// Endpoint:
//
...
func (h *Handler) ConsumeCodexResetCredit(c *gin.Context) {
	...
	// list and pick the first credit whose status is "available".
	if creditID == "" {
		...
		id, found := firstAvailableCreditID(listBody)
		if !found {
			log.Warnf("codex reset-credits consume aborted: no available credit auth_index=%s", authIndex)
			c.JSON(http.StatusConflict, gin.H{"error": "no available reset credit to consume"})
			return
		}
		creditID = id
	}

	reqBody, err := json.Marshal(map[string]string{"credit_id": creditID})
	if err != nil {
...
// firstAvailableCreditID returns the id of the first credit whose status is
// "available" from a rate-limit-reset-credits list response.
func firstAvailableCreditID(body []byte) (string, bool) {
	var parsed struct {
		Credits []struct {
			ID     string `json:"id"`
	...
	for _, credit := range parsed.Credits {
		if strings.EqualFold(strings.TrimSpace(credit.Status), "available") && strings.TrimSpace(credit.ID) != "" {
			return strings.TrimSpace(credit.ID), true
		}
	}
	return "", false
}
